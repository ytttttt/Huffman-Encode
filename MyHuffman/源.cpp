...
using namespace std;
//1 定义常量，定义一个无穷量，定义编码位最多有0010 0000(2^5)个，霍夫曼节点和编码字最多有0100 0000 0000(2^10)个
...
//编码
//Huffman编码方式为每个字符编码是在节点数组huffmanNode中从该字符节点开始，向上追溯其父节点，若该节点是其父节点的左子节点，则其霍夫曼编码的[n-1]位是0，若是其父节点的右子节点，则其霍夫曼编码的【n-1】位是1；再对父节点找父节点，一直向上追溯，将start--，将其每一位编码的bit值弄清楚，直到其父节点不存在（parent=-1即到达根节点）。因为对每一个字符其start都设为n-1（因为n个字符的霍夫曼编码最长为n-1），其每一位bit值是从后往前追溯，此时的start值表明输出的霍夫曼编码应从第start位开始输出。因为前面的值没有确定了。只有从start当前值到n-1才是真正的编码值。
...
//解码
//对0/1序列s的bit位，从s[0]-s.length()从霍夫曼树的根节点开始往下遍历直到找到叶子节点，将其叶子节点的字符（原码）存入容器，找到一个原码后接着再对当前的s[i]从根节点向下遍历，每到达一个叶子节点就将字符存入容器，直到遍历到序列末端，输出容器中的值即是对应的原码。或是到达序列末端却发现缺失，则中断解码。
